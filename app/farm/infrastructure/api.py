...
from app.farm.domain.schemas import FarmCreate, FarmResponse, FarmListResponse
...
@router.get("/list", response_model=FarmListResponse, status_code=status.HTTP_200_OK)
def list_farms(
    db: Session = Depends(getDb),
    current_user: UserInDB = Depends(get_current_user)
):
    ...
    try:
        return list_farms_use_case.execute(current_user)
    except DomainException as e:
        raise e
    except Exception as e:
        raise HTTPException(
