        ...
        for plot in plots:
            ...
            for task in plot_tasks:
                ...
                plot_task_costs.append(TaskCost(
                    ...
                    insumo=task.insumos.insumo.nombre if task.insumos and task.insumos.insumo else None,
                    cantidad_insumos=task.insumos.cantidad_utilizada if task.insumos else 0,
                    unidad_insumos=task.insumos.insumo.unidad_medida.abreviatura if task.insumos and task.insumos.insumo and task.insumos.insumo.unidad_medida else None,
                    precio_unitario_insumos=task.insumos.insumo.costo_unitario if task.insumos and task.insumos.insumo else 0,
                    costo_insumos=input_cost,
                    maquinaria=task.maquinarias.maquinaria.nombre if task.maquinarias and task.maquinarias.maquinaria else None,
                    horas_maquinaria=task.maquinarias.horas_uso if task.maquinarias else 0,
                    precio_hora_maquinaria=task.maquinarias.maquinaria.costo_hora if task.maquinarias and task.maquinarias.maquinaria else 0,
